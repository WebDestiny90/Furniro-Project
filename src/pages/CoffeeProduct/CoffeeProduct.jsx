...
const CoffeeProduct = () => {
  ...
  const handleColorChange = (color) => {
    ...
    setMainImage(coffee.colors[color].main);
  };
  ...
  const handleAddToCart = () => {
    ...
    addToCart({ ...coffee, category: 'coffee' }, currentColor, 1);
    setShowNotif(true);
  };
  ...
  const handleWishListClick = () => {
    ...
    if (isInWishList(coffee.id, 'coffee')) {
      removeFromWishList(coffee.id, 'coffee');
    } else {
      addToWishList({ ...coffee, category: 'coffee' });
  ...
  return (
    <>
          ...
          <div className="singleImageBox">
            <div className="singleImageBoxItemsContainer">
              <div className="singleImageBoxItems">
                {coffee.colors[currentColor].previews.map((preview, index) => (
                  <img
                    key={preview}
                    ...
                    height={80}
                    alt={coffee.alt}
                    onClick={() => handleImageClick(coffee.colors[currentColor].mains[index])}
            ...
            <div className="singleDescription">
              <h1 className="descriptionTitle">{coffee.name}</h1>
              <p className="descriptionPrice">{coffee.price}</p>
              ...
              <p className="descriptionText">{coffee.descriptionText}</p>
              <div className="descriptionColorBox">
                <p className="descriptionColorText">Color</p>
                {Object.entries(coffee.colors).map(([colorName, colorData]) => (
                  <button
                    key={colorName}
                ...
                <div className="cartButton">
                  ...
                  <svg
                    className={`wishList_Icon ${isInWishList(coffee.id, 'coffee') ? 'active' : ''}`}
                    viewBox="0 0 24 24"
                    fill="none"
      ...
      <ProductDescription product={coffee} />
