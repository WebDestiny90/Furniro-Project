...
const TvStandProduct = () => {
  const { id } = useParams();
  const { addToCart } = useCart();
  const { isAuth } = useAuth();
  const { addToWishList, removeFromWishList, isInWishList } = useWishList();
  const [tvstands, setTvstands] = useState(null);
  ...
  const handleColorChange = (color) => {
    ...
    setMainImage(tvstands.colors[color].main);
  };
  ...
  const handleAddToCart = () => {
    ...
    addToCart(tvstands, currentColor, 1);
    setShowNotif(true);
  };
  ...
  const handleWishListClick = () => {
    ...
    if (isInWishList(tvstands.id)) {
      removeFromWishList(tvstands.id);
    } else {
      addToWishList(tvstands);
  ...
  return (
    <>
          ...
          <div className="singleImageBox">
            <div className="singleImageBoxItemsContainer">
              <div className="singleImageBoxItems">
                {tvstands.colors[currentColor].previews.map((preview, index) => (
                  <img
                    key={preview}
                    ...
                    height={80}
                    alt={tvstands.alt}
                    onClick={() => handleImageClick(tvstands.colors[currentColor].mains[index])}
            ...
            <div className="singleDescription">
              <h1 className="descriptionTitle">{tvstands.name}</h1>
              <p className="descriptionPrice">{tvstands.price}</p>
              ...
              <p className="descriptionText">{tvstands.descriptionText}</p>
              <div className="descriptionColorBox">
                <p className="descriptionColorText">Color</p>
                {Object.entries(tvstands.colors).map(([colorName, colorData]) => (
                  <button
                    key={colorName}
                ...
                <div className="cartButton">
                  ...
                  <svg
                    className={`wishList_Icon ${isInWishList(tvstands.id) ? 'active' : ''}`}
                    viewBox="0 0 24 24"
                    fill="none"
      ...
      <ProductDescription product={tvstands} />
