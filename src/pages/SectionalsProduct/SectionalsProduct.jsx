...
import "../SofaProduct/SofaProduct.css";

const SectionalsProduct = () => {
  const { id } = useParams();
  const { addToCart } = useCart();
  const { isAuth } = useAuth();
  const { addToWishList, removeFromWishList, isInWishList } = useWishList();
  const [sectionals, setSectionals] = useState(null);
  ...
  const handleColorChange = (color) => {
    ...
    setMainImage(sectionals.colors[color].main);
  };
  ...
  const handleAddToCart = () => {
    ...
    addToCart({ ...sectionals, category: 'sectional' }, currentColor, 1);
    setShowNotif(true);
  };
  ...
  const handleWishListClick = () => {
    ...
    if (isInWishList(sectionals.id, 'sectional')) {
      removeFromWishList(sectionals.id, 'sectional');
    } else {
      addToWishList({ ...sectionals, category: 'sectional' });
  ...
  return (
    <>
          ...
          <div className="singleImageBox">
            <div className="singleImageBoxItemsContainer">
              <div className="singleImageBoxItems">
                {sectionals.colors[currentColor].previews.map((preview, index) => (
                  <img
                    key={preview}
                    ...
                    height={80}
                    alt={sectionals.alt}
                    onClick={() => handleImageClick(sectionals.colors[currentColor].mains[index])}
            ...
            <div className="singleDescription">
              <h1 className="descriptionTitle">{sectionals.name}</h1>
              <p className="descriptionPrice">{sectionals.price}</p>
              ...
              <p className="descriptionText">{sectionals.descriptionText}</p>
              <div className="descriptionColorBox">
                <p className="descriptionColorText">Color</p>
                {Object.entries(sectionals.colors).map(([colorName, colorData]) => (
                  <button
                    key={colorName}
                ...
                <div className="cartButton">
                  ...
                  <svg
                    className={`wishList_Icon ${isInWishList(sectionals.id, 'sectional') ? 'active' : ''}`}
                    viewBox="0 0 24 24"
                    fill="none"
      ...
      <ProductDescription product={sectionals} />
...
export default SectionalsProduct;